...
document.addEventListener('DOMContentLoaded', () => {
    ...
    accHead.forEach((head, i) => {
        head.addEventListener('click', () => {
            let isCollapsed = accBody[i].getAttribute('data-collapsed') === 'true';
            
            if(isCollapsed) {
                expandSection(accBody[i])
                accBody[i].setAttribute('data-collapsed', 'false');
                accItem[i].classList.toggle('faq-item--active')
            } else {
                collapseSection(accBody[i]);
                accItem[i].classList.toggle('faq-item--active')
            }
        })
    })
